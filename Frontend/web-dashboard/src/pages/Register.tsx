...
export const Register: React.FC = () => {
  const { formData, setFormData, status, setStatus, security, setSecurity, passwordStrength, validate } = useRegister();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus(s => ({
        ...s,
        loading: false,
        error: isRateLimited ? "Too many attempts. Wait 60s." : (err as Error).message || "Registration failed",
      }));
    }
  };
    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 flex-col justify-between p-12 text-white">
        ...
        <div className="space-y-8">
          <div>
            <h2 className="text-4xl font-bold leading-tight">The smarter way to run email campaigns</h2>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <InputField icon={<User size={16} />} label="Full Name" name="name"
                value={formData.name} onChange={setFormData} placeholder="John Doe" />

              <InputField icon={<Mail size={16} />} label="Email address" name="email" type="email"
                value={formData.email} onChange={setFormData} placeholder="john@example.com" />

              {/* Password */}
              <div className="relative">
                <InputField icon={<Lock size={16} />} label="Password" name="password"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => { confirmPasswordRef.current = e.target.value; setConfirmDisplay(e.target.value); }}
              ...
              {/* Submit */}
              <button type="submit" disabled={status.loading || security.cooldown > 0}
                ...
                {status.loading ? <Loader className="animate-spin w-5 h-5" />
                  : security.cooldown > 0 ? <><Clock size={16} /> Wait {security.cooldown}s</>
                  : "Create Account"}
              </button>
            </form>
          </div>
...
const InputField: React.FC<InputFieldProps> = ({ label, icon, name, value, onChange, error, ...props }) => (
  <div className="flex flex-col gap-1.5">
...
const SuccessView: React.FC<{ email: string }> = ({ email }) => (
  <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
    <div className="w-full max-w-md bg-white px-8 py-12 rounded-2xl shadow-sm border border-gray-200 text-center">
      ...
      <Link to="/login" className="mt-8 block w-full py-3 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-colors">
        Go to Login
      </Link>
    </div>
  </div>
